/**
 * Compare + sync Saturday 11 Jul 2026 schedule with event students.
 *
 * Dry run (default):
 *   npm run sync:july11
 *
 * Create event (copy fields from 5 Jul) + replace roster:
 *   npm run sync:july11 -- --create-event --sync
 */
...
} from "../seed/cosmic-concert-2026-07-11-schedule";
...
async function resolveEventId(
  ...
  if (!createEvent && !sync) {
    ...
    console.log(
      "\n  npm run sync:july11 -- --create-event --sync   → create event + insert roster"
    );
    return "";
  }
...
async function main() {
  ...
  if (!sync) {
    console.log("\nDry run complete.");
    console.log(
      "  npm run sync:july11 -- --create-event --sync   → create event + replace roster"
    );
    return;
  }
