/**
 * Compare + sync Sunday 5 Jul 2026 schedule with event students.
 *
 * Dry run (default):
 *   npm run sync:july5
 *
 * Create event if missing, then replace roster:
 *   npm run sync:july5 -- --create-event --sync
 */
...
} from "../seed/cosmic-concert-2026-07-05-schedule";
...
async function resolveEventId(
  ...
  if (!createEvent && !sync) {
    ...
    console.log("\nFirst 10 on schedule:");
    SCHEDULE_STUDENTS.slice(0, 10).forEach((s, i) => {
      const nick = s.nickname ? ` (${s.nickname})` : "";
      console.log(
        `${String(i + 1).padStart(2, " ")}. [${s.slot}] ${s.full_name}${nick} — ${s.teacher_name}`
      );
    });
    console.log(`... and ${SCHEDULE_STUDENTS.length - 10} more`);
    console.log(
      "\n  npm run sync:july5 -- --create-event --sync   → create event + insert roster"
    );
    return "";
  }
...
async function main() {
  ...
  if (!sync) {
    console.log("\nDry run complete.");
    console.log(
      "  npm run sync:july5 -- --create-event --sync   → create event + replace roster"
    );
    return;
  }
